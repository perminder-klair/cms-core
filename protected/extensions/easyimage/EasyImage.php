...
class EasyImage extends CApplicationComponent
{
	...
	public $quality = 100;
	public $cachePath = '/files/cache/'; //relative web root (recommended: /assets/easyimage/)
	...
	public function smart_resize($width = NULL, $height = NULL)
    {
	    $image = $this->_image;
	    
        if ($image->width/$image->height > $width/$height) {
            $newImage = $this->resize($width, $height, Image::HEIGHT);
        }
        else {
        	$newImage = $this->resize($width, $height, Image::WIDTH);
        }
